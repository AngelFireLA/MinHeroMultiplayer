 ...
 * ValueDict object.
 */
class ValueDict extends _value.Value {
  /**
   * Value type.
   */
  static TYPE = 'dict';
  ...
  static get CHILD_TAG_NAMES() {
    if (!childTagNames) {
      ...
      for (const ValueType of [_array.ValueArray, _boolean.ValueBoolean, _data.ValueData, ValueDict, _integer.ValueInteger, _real.ValueReal, _string.ValueString]) {
  ...
  fromXmlElement(element) {
    (0, _util.assertXmlTagName)(element, 'dict');
    ...
    for (let i = 0; i < l; i += 2) {
      ...
      (0, _util.assertXmlTagName)(eK, 'key');
      const key = (0, _util.xmlElementText)(eK)?.nodeValue || '';
  ...
  toXml(options = null, depth = 0) {
    const i = options?.indentString ?? _options.INDENT_STRING;
