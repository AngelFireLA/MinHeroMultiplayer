import { stringEncode } from "./swf/util.mjs";
import { Tag } from "./swf/tag.mjs";
import { Swf } from "./swf/swf.mjs";
...
function bytecodeLoadMovieSwf4(url) {
  return concat([asmv1ActionPush([asvm1TypeString(url)]), asmv1ActionPush([asvm1TypeString('/')]), asvm1ActionLoadMovie(), asvm1ActionEnd()]);
}
...
function bytecodeLoadMovieSwf5(url) {
  return concat([asmv1ActionConstantPool([url, '_level0']), asmv1ActionPush([asvm1TypeConstant8(0), asvm1TypeConstant8(1)]), asvm1ActionLoadMovie(), asvm1ActionEnd()]);
}
 ...
 * @returns Movie data.
 */
export function loader(swfv, width, height, fps, color, url, delay = 0) {
  if (swfv < 4) {
