...
import { htmlEncode } from "../util.mjs";
...
export class ProjectorHtml extends Projector {
  ...
  getHtmlDefault() {
    ...
    return ['<!DOCTYPE html>', `<html${hAttr}>`, ' <head>', '  <meta charset="UTF-8">', '  <meta http-equiv="X-UA-Compatible" content="IE=Edge">', ...(title === null ? [] : [`  <title>${htmlEncode(title)}</title>`]), '  <style>', '   * {', '    margin: 0;', '    padding: 0;', '   }', '   html,', '   body {', '    height: 100%;', '   }', '   body {', ...(background === null ? [] : [`    background: ${htmlEncode(background)};`]), ...(color === null ? [] : [`    color: ${htmlEncode(color)};`]), '    font-family: Verdana, Geneva, sans-serif;', '   }', '   object,', '   embed {', '    display: block;', '    outline: 0;', '   }', '   object:focus,', '   embed:focus {', '    outline: 0;', '   }', '   .main {', '    display: table;', '    height: 100%;', '    width: 100%;', '   }', '   .player {', '    display: table-cell;', '    vertical-align: middle;', '   }', '   .player object,', '   .player embed {', '    margin: 0 auto;', '   }', '  </style>', ' </head>', ' <body>', '  <div class="main">', '   <div class="player">', '    <object', ...[...object.entries()].map(([a, v]) => `     ${a}="${htmlEncode(v, true)}"`), '    >', ...[...param.entries()].map(([a, v]) => `     <param name="${a}" value="${htmlEncode(v, true)}">`), '     <embed', ...[...embed.entries()].map(([a, v]) => `      ${a}="${htmlEncode(v, true)}"`), '     >', '    </object>', '   </div>', '  </div>', ' </body>', '</html>', ''].map(s => s.replace(/^\s+/, s => '\t'.repeat(s.length))).join('\n');
  }
}
