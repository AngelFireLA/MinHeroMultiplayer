...
import { ArchiveDir, createArchiveByFileExtensionOrThrow, PathType } from '@shockpkg/archive-files';
import { Plist, ValueDict, ValueString } from '@shockpkg/plist-dom';
import { trimExtension } from "../../util.mjs";
import { macProjectorMachoPatch } from "../../util/mac.mjs";
...
export class ProjectorSaMac extends ProjectorSa {
  ...
  getBundleName() {
    ...
    return bundleName === true ? trimExtension(basename(this.path), this.extension, true) : bundleName;
  }
    ...
    const patch = {
      ...
      after: async () => {
        ...
        const oldDict = async () => {
          if (!oldDictValue) {
            const plist = new Plist();
            plist.fromXml(xmlOld ?? (await readFile(this.infoPlistPath, 'utf8')));
        ...
        if (appIconName || iconData) {
          const oldIconName = (await oldDict()).getValue('CFBundleIconFile').castAs(ValueString).value;
          ...
          if (appIconName) {
            ...
            tasks.push(async () => rename(iconPathOld, iconPathNew).then(async () => {
              if (iconData) {
                await writeFile(iconPathNew, iconData);
              }
            }));
          } else if (iconData) {
            tasks.push(async () => writeFile(iconPathOld, iconData));
          }
        }
