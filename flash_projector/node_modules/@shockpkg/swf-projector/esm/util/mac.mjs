...
import { launcher } from "../util.mjs";
import { hex4, align, concat, getCstrN } from "./internal/data.mjs";
import { slider } from "./internal/patch.mjs";
import { VM_PROT_READ, FAT_MAGIC, MH_MAGIC, MH_CIGAM, MH_MAGIC_64, MH_CIGAM_64, LC_SEGMENT, LC_SYMTAB, LC_DYSYMTAB, LC_SEGMENT_64, LC_CODE_SIGNATURE, LC_SEGMENT_SPLIT_INFO, LC_DYLD_INFO, LC_DYLD_INFO_ONLY, LC_FUNCTION_STARTS, LC_DATA_IN_CODE, LC_DYLIB_CODE_SIGN_DRS, LC_LINKER_OPTIMIZATION_HINT, LC_DYLD_EXPORTS_TRIE, LC_DYLD_CHAINED_FIXUPS, SEG_TEXT, SECT_TEXT, SEG_LINKEDIT, CPU_TYPE_POWERPC, CPU_TYPE_POWERPC64, CPU_TYPE_I386, CPU_TYPE_X86_64, CPU_TYPE_ARM64 } from "./internal/mac/constants.mjs";
import { macProjectorTitlePatchesByCpuType } from "./internal/mac/title.mjs";
 ...
 */
export function machoTypesData(data) {
  let le = false;
 ...
 * @returns Mach-O types.
 */
export async function machoTypesFile(path) {
  let data;
 ...
 * @returns FAT binary.
 */
export function machoFat(machos) {
 ...
 */
export function machoThins(data) {
...
function macProjectorMachoPatchEach(data, title) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (commandV.getUint32(0, le) === SEGMENT && getCstrN(command, 8, 16) === SEG_LINKEDIT) {
      if (linkeditI > 0) {
  ...
  const macho = concat([header, ...commands, data.subarray(commands.reduce((v, c) => v + c.length, header.length), Number(fileoff)), new Uint8Array(secdata), new Uint8Array(segSize - secdata.byteLength), data.subarray(Number(fileoff))]);

  // Find the text section.
  let textSegment = null;
  for (const command of commands) {
    const commandV = new DataView(command.buffer, command.byteOffset, command.byteLength);
    if (commandV.getUint32(0, le) === SEGMENT && getCstrN(command, 8, 16) === SEG_TEXT) {
      if (textSegment) {
  ...
  for (const Patcher of patchers) {
    ...
    if (patcher.check()) {
      if (found) {
        throw new Error(`Duplicate patcher for CPU type: 0x${hex4(cpuType)}`);
      }
      found = patcher;
    }
  }
 ...
 */
export function macProjectorMachoPatch(macho, options) {
  const {
    removeCodeSignature,
    patchWindowTitle
  } = options;
 ...
 */
export async function machoAppLauncherThin(type) {
  const {
    cpuType
  } = type;
  switch (cpuType) {
 ...
 * @returns Launcher data.
 */
export async function machoAppLauncherFat(types) {
 ...
 * @returns Launcher data.
 */
export async function machoAppLauncher(types) {
