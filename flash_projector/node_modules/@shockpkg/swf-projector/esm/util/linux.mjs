...
import { align, findIndex } from "./internal/data.mjs";
import { decode, Elf32Shdr, Elf64Shdr, PT_LOAD, SHF_ALLOC, SHT_NOBITS, SHT_PROGBITS, SHT_STRTAB } from "./internal/linux/elf.mjs";
import { title64 } from "./internal/linux/title64.mjs";
import { title32 } from "./internal/linux/title32.mjs";
import { menu64 } from "./internal/linux/menu64.mjs";
import { menu32 } from "./internal/linux/menu32.mjs";
import { path64 } from "./internal/linux/path64.mjs";
import { path32 } from "./internal/linux/path32.mjs";
import { offset64 } from "./internal/linux/offset64.mjs";
...
function linuxProjectorAddSection(elf, data) {
  ...
  for (const phdr of elf.programHeaders) {
    if (phdr.pType === PT_LOAD && (phdrLoadLast === null || phdr.pVaddr >= phdrLoadLast.pVaddr)) {
      phdrLoadLast = phdr;
    }
  }
  if (!phdrLoadLast) {
  ...
  if (elf.bits !== 64) {
    for (const shdr of elf.sectionHeaders) {
      ...
      allowBssAfterLoadData = findIndex(new Uint8Array(shdr.data), te.encode('libgtk-1.2.so.0')) >= 0;
    }
  }
  ...
  for (let i = 0; i < elf.sectionHeaders.length; i++) {
    ...
    if (shdr.shAddr) {
      ...
      if (!(allowBssAfterLoadData && shdr.shType === SHT_NOBITS) && add(shdr.shOffset, shdr.fileSize) > origPhdrLoadEndF) {
 ...
 */
export function linuxProjectorPatch(elf, options) {
  const {
    patchWindowTitle,
    patchMenuRemove,
    patchProjectorPath,
  ...
  if (typeof patchWindowTitle === 'string') {
    ...
    const patches = e.bits === 64 ? title64.map(Patch => new Patch(e, titleA, titleL)) : title32.map(Patch => new Patch(e, titleA, titleL));
    patchers.push(['Window Title', patches]);
  }
  if (patchMenuRemove) {
    const patches = e.bits === 64 ? menu64.map(Patch => new Patch(e)) : menu32.map(Patch => new Patch(e));
    patchers.push(['Menu Remove', patches]);
  }
  if (patchProjectorPath) {
    const patches = e.bits === 64 ? path64.map(Patch => new Patch(e)) : path32.map(Patch => new Patch(e));
 ...
 */
export async function linuxLauncher(type) {
  switch (type) {
    case 'i386':
      {
