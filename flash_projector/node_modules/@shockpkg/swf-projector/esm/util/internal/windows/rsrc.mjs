import { Resource, Data } from '@shockpkg/resedit';
 ...
 */
export function rsrcPatchIcon(rsrc, iconData) {
  const ico = Data.IconFile.from(iconData.buffer.slice(iconData.byteOffset, iconData.byteLength));
  for (const iconGroup of Resource.IconGroupEntry.fromEntries(rsrc.entries)) {
    Resource.IconGroupEntry.replaceIconsForResource(rsrc.entries, iconGroup.id, iconGroup.lang, ico.icons.map(icon => icon.data));
  }
}
 ...
 */
export function rsrcPatchVersion(rsrc, versionStrings) {
