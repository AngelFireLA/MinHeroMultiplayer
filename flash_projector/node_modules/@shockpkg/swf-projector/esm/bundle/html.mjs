...
import { ProjectorHtml } from "../projector/html.mjs";
import { htmlEncode, trimExtension } from "../util.mjs";
...
export class BundleHtml extends Bundle {
  ...
  getLauncher() {
    ...
    return ['<!DOCTYPE html>', `<html${hAttr}>`, ' <head>', '  <meta charset="UTF-8">', '  <meta http-equiv="X-UA-Compatible" content="IE=Edge">', `  <meta http-equiv="refresh" content="0; url=${htmlEncode(url, true)}">`, ...(title === null ? [] : [`  <title>${htmlEncode(title)}</title>`]), ' </head>', ' <body></body>', '</html>', ''].map(s => s.replace(/^\s+/, s => '\t'.repeat(s.length))).join('\n');
  }
