...
exports.patchWindowTitleData = patchWindowTitleData;
exports.patchWindowTitleRsrc = patchWindowTitleRsrc;
var _resedit = require("@shockpkg/resedit");
var _data = require("../data.js");
var _patch = require("../patch.js");
var _constants = require("./constants.js");
var _exe = require("./exe.js");
/**
 ...
 */
function patchWindowTitleRsrc(rsrc, title) {
  // Match all known titles.
  const titleMatch = /^((Shockwave )?Flash|(Adobe|Macromedia) Flash Player \d+([,.]\d+)*)$/;
  ...
  for (const entry of rsrc.entries) {
    ...
    const table = _resedit.Resource.StringTable.fromEntries(entry.lang, [entry]);
    for (const {
      text
    } of table.getAllStrings()) {
    ...
    if (titleStringTableId === null) {
      ...
      if (strings.length === 2 && strings[0] === 'Shockwave Flash' && strings[1].startsWith('Shockwave Flash ')) {
        titleStringTableId = entry.id;
      }
    }
    if (titleStringTableId === null) {
...
function patchWindowTitleData(exe, address) {
  ...
  const dataSections = exe.getAllSections().filter(({
    ...
    !(characteristics & _constants.IMAGE_SCN_MEM_READ) ||
    // eslint-disable-next-line no-bitwise
    characteristics & _constants.IMAGE_SCN_MEM_WRITE ||
    // eslint-disable-next-line no-bitwise
    characteristics & _constants.IMAGE_SCN_MEM_EXECUTE ||
    // eslint-disable-next-line no-bitwise
    characteristics & _constants.IMAGE_SCN_MEM_SHARED) {
      return false;
    }
    for (const a of excluded) {
      if (a >= virtualAddress && a < virtualAddress + virtualSize) {
        return false;
      }
    }
    return true;
  ...
  const titles = [['Adobe Flash Player ', /^Adobe Flash Player \d+([,.]\d+)*$/]];
  let oldAddress = 0;
  for (const {
    info: {
      virtualAddress
    ...
    for (const [pre, reg] of titles) {
      const pred = (0, _data.encodeUtf16)(pre, true);
      for (const index of (0, _patch.findExact)(d, pred)) {
        const s = (0, _data.getUtf16)(d, index, true);
        if (!s || !reg.test(s)) {
          continue;
        }
  ...
  if (newHeader.is32bit()) {
    ...
    for (const [o, patch] of patches) {
      for (const i of (0, _patch.findFuzzy)(data, patch)) {
        const off = i + o;
