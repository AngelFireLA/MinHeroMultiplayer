...
exports.Entry = exports.Archive = void 0;
...
const pipe = (0, _nodeUtil.promisify)(_nodeStream.pipeline);
/**
 * Entry object.
 */
class Entry {
  ...
  constructor(info) {
    ...
    this.path = (0, _util.pathNormalize)(info.pathRaw);
  ...
  async read() {
    ...
    return stream ? (0, _util.streamToBuffer)(stream) : null;
  }
  ...
  async setAttributes(path, pathFull = null, options = {}) {
    ...
    let link = false;
    switch (type) {
      case _types.PathType.FILE:
        {
          break;
        }
      case _types.PathType.RESOURCE_FORK:
        {
          break;
        }
      case _types.PathType.DIRECTORY:
        {
          break;
        }
      case _types.PathType.SYMLINK:
        {
          link = true;
          break;
        }
    ...
    if (!ignorePermissions && mode !== null) {
      const chmod = link ? _util.fsLchmod : _util.fsChmod;
      const modeSet = (0, _util.modePermissionBits)(mode);
      await chmod(pathSet, modeSet);
    }
    if (!ignoreTimes && atimeSet && mtimeSet) {
      const utimes = link ? _util.fsLutimes : _util.fsUtimes;
  ...
  async _extractStreamToFile(path, reader, options) {
    ...
    const stat = await (0, _util.fsLstatExists)(path);
    if (stat) {
    ...
    // Write file.
    await (0, _promises.writeFile)(path, Buffer.alloc(0));
    const stream = await reader();
    if (stream) {
      await pipe(stream, (0, _nodeFs.createWriteStream)(path));
    }
  ...
  async _extractResourceFork(path, options) {
    ...
    const pathRsrc = (0, _util.pathResourceFork)(path);
    ...
    if (stream) {
      await pipe(stream, (0, _nodeFs.createWriteStream)(pathRsrc));
    } else {
      await (0, _promises.writeFile)(pathRsrc, Buffer.alloc(0));
    }
  ...
  async _extractSymlink(path, options) {
    ...
    // eslint-disable-next-line unicorn/prefer-ternary
    if (symlinkAsFile) {
      await (0, _promises.writeFile)(path, target);
    } else {
      await (0, _util.fsSymlink)(path, target);
    }
...
class Archive {
  ...
  afterReadSetAttributes(path, entry, options = {}) {
    ...
    afters.set((0, _nodePath.resolve)(path), {
      path,
      entry,
      options
    });
  ...
  afterReadSetAttributesRemove(path) {
    ...
    afters.delete((0, _nodePath.resolve)(path));
  }
