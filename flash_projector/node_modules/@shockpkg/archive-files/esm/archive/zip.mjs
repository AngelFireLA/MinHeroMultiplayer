...
import { modeToPathType, streamToBuffer } from "../util.mjs";
...
export class ArchiveZip extends Archive {
  ...
  async _read(itter) {
    ...
    const each = async yentry => {
      ...
      const readData = type === PathType.FILE ? async () => yauzlEntryRead(zipfile, yentry) : null;
      const readSymlink = type === PathType.SYMLINK ? async () => yauzlEntryReadSymlink(zipfile, yentry) : null;
